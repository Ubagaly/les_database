...
START TRANSACTION;
INSERT INTO sample.users (SELECT * FROM test_baza.users WHERE test_baza.users.id = 1);
COMMIT;
...
BEGIN
  IF NEW.name IS NULL AND NEW.description IS NULL
    THEN SIGNAL sqlstate '45001' set message = "value name and description can not be NULL"; 
  end if;
END; //
